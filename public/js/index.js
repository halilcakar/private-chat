...
var userID, channel, joining = false;
...
function sendMessage() {
    socket.emit('newMessage', { id: channel.id, message: $('#new-message').val().trim() });
    $('#new-message').val('').focus();
}

function showForm(channelData, success = true) {
    if (!success) {
        $('#channel-form').append('<div class="alert alert-danger mt-3" role="alert">' + channelData + '</div >');
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (location.hash) {
        ...
        $createChannel.on('click', function (e) {
            socket.emit('joinChannel', { username: username.value, channelID: location.hash.replace('#channel=', '') }, showForm);
        });
    }
    else {
        $createChannel.on('click', function (e) {
